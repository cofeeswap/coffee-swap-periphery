...
import CoffeeRouter01 from '../../build/CoffeeRouter01.json'
import CoffeeMigrator from '../../build/CoffeeMigrator.json'
import CoffeeRouter02 from '../../build/CoffeeRouter02.json'
...
export async function v2Fixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<V2Fixture> {
  ...
  // deploy routers
  const router01 = await deployContract(wallet, CoffeeRouter01, [factoryV2.address, WETH.address], overrides)
  const router02 = await deployContract(wallet, CoffeeRouter02, [factoryV2.address, WETH.address], overrides)

  // event emitter for testing
  const routerEventEmitter = await deployContract(wallet, RouterEventEmitter, [])

  // deploy migrator
  const migrator = await deployContract(wallet, CoffeeMigrator, [factoryV1.address, router01.address], overrides)
